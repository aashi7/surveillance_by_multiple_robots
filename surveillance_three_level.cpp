...
class G_T
{
	public:
		struct node 
		{
			...
			node* parent;
			float cost; // cost of the edge between this node and its parent   
		};
...
G_T::node* G_T::add_vertex(node* parent, vector<int>& new_q, int robot_id)
{
	...
	new_node->cost = search_in_GM(new_q, robot_id);     // which robot was allocated   // construct a copy 
...
void print_GT_node(G_T::node* node_to_print)
{
	...
	for (int i = 0; i < q.size(); i++)
		cout << q[i] << " ";
	cout << "Cost: " << node_to_print->cost << endl; 
	return; 
}
...
float G_T::construct_GT_upto_goal()
{
	...
	while (not goal_expanded && not to_expand.empty())
	{
		...
		for (int i=0; i < successors.size(); i++)
		{
			...
			float g = g_parent + (successors[i]->cost - g_parent); 
			to_expand.push({g, successors[i]});
